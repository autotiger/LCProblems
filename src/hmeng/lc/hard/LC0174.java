...
public class LC0174 {
    ...
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        for (int row = rowLen-1; row >= 0; row--) {
            for (int col = colLen-1; col >=0; col--) {
                int healthNeeded = Math.min(dp[row+1][col], dp[row][col+1]) - dungeon[row][col];
                dp[row][col] = healthNeeded <= 0 ? 1 : healthNeeded;
            }
        }
    ...
    public static void main(String[] args) {
        LC0174 lc = new LC0174();
        int[][] dungeon = {{-2,-3,3},{-5,-10,1},{10,30,-5}};
        System.out.println(lc.calculateMinimumHP(dungeon));
    }
